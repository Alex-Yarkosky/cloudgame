...
public class Cloud {
	/**
	 * Lightning l created to allow interactions between Cloud & Lightning classes
	 * int x used to keep track of Cloud's x location final final
	 * int Y used to define the Cloud's permanent y location final
	 * int SPD used to change Cloud's x location 
	 * final int DIAM used to define the diameter of each circle used to create the
	 * entire Cloud
	 * object boolean isMoving used to determine if the user has begun the game &
	 ...
	 * 
	 * @param lightningBoltOne
	 *            used for interactions between a first Lightning & Cloud classes
	 * @param lightningBoltTwo
	 * 			  used for interactions between a second Lightning & Cloud classes
	 */
	 ...
	 * 
	 * @param g
	 *            used to create the shapes used in creating the Cloud object
	 *            visual
	 */
	public void paint(Graphics g) {
		...
		if(cloudSegments >= 3)
			g.fillOval(x + 15, Y - 15, DIAM, DIAM);
		g.setColor(Color.GRAY);
		if(cloudSegments >= 2)
			g.fillOval(x + 15, Y + 15, DIAM, DIAM);
		g.setColor(Color.WHITE);
		if(cloudSegments >= 4)
			g.fillOval(x + 30, Y, DIAM, DIAM);
		if(cloudSegments == 6)
			g.fillOval(x + 45, Y - 15, DIAM, DIAM);
		g.setColor(Color.GRAY);
		if(cloudSegments >= 5)
		g.fillOval(x + 45, Y + 15, DIAM, DIAM);
		if(!isMoving){
	...
	/**
	 * Checks if the Cloud is moving & then changes the Cloud's x variable
	 * according to the parameter
	 * 
	 * @param mouseX
	 *            used to move Cloud object towards the x location of the mouse
	 */
	...
	/**
	 * Checks to see if the Cloud is out of the bounds of being visually
	 * displayed & if so moves it fully to the other side of the screen
	 */
	...
	/**
	 * Checks to see if the first Lightning class bolt has hit the Cloud object
	 * based off of their x locations
	 */
	public void checkFirstStrike() {
		...
		if (isMoving) {
			if ((l1.getX() + 10 >= x && l1.getX() <= x)
					...
					|| ((x <= l1.getX() && x + additional >= l1.getX()) && (x <= l1.getX() && x + additional >= l1
							.getX()))) {
	...
	/**
	 * Checks to see if the second Lightning class bolt has hit the Cloud object
	 * based off of their x locations
	 */
	public void checkSecondStrike() {
		...
		if (isMoving) {
			if ((l2.getX() + 10 >= x && l2.getX() <= x)
					...
					|| ((x <= l2.getX() && x + 60 >= l2.getX()) && (x <= l2.getX() && x + 60 >= l2
							.getX()))) {
	...
	/**
	 * Checks if the Cloud is currently immune to being struck by a bolt &
	 * if so counts to when it is no longer immune
	 */
	...
	/**
	 * Allows the cloud to start moving at the user's chosen beginning of the
	 * game
	 */
