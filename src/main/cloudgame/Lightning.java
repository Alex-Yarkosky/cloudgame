...
public class Lightning {
  /**
   * int x used to store the bolt's x location int wait used to wait before a new bolt is created &
   * have a period without that bolt final Y used to store the bolt's y location int newX used to
   * create a new x location for a new bolt boolean alert used to know when to alert the user of an
   * upcoming new bolt final int DIAM used to define the diameter of the displayed warning arcs of
   * upcoming bolts Color c used to define the color of the bolt
   */
  private int x;
   ...
   *
   * @param g used to create the shapes used in creating the Lightning object visual
   */
  public void paint(Graphics g) {
    g.setColor(c);
    if (alert) g.fillArc(newX - DIAM / 2, Y - DIAM / 2, DIAM, DIAM, 180, 180);
